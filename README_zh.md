...
- **全部重构**：架构更清晰，行为更稳定，跨环境一致性更好。
- **tmux 完美支持**：稳定分屏、窗格标题/边框/状态栏，并且主题只在 CCB 运行期间启用，退出自动恢复。
- **支持任何终端**：只要能运行 tmux 就能获得完整多模型分屏体验（Windows 原生除外，仍强烈建议通过 WSL/WezTerm 统一迁移到 tmux）。
